...
void FW_UPDATE_MasterStart(void)
{
    ...
    if (FW_UPDATE_MASTER_STATE_IDLE == MasterCtrl.State) {
      ...
      if(state_tick && clock_time_exceed(state_tick, FW_UPDATE_DURATION)){
        ...
        printf("[OTA][FLW] fw update time out%d\n",__LINE__);
        return;
      }
        if (Msg) {
      ...
      printf("[OTA][FLW] master fw update success\n");
      ...
      printf("[OTA][FLW] master fw update failed\n");
#if (APP_FLASH_PROTECTION_ENABLE)
      flash_prot_op_cb(FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_END, 0, 0);
#endif
...
//        }
        printf("[OTA][FLW] slave fw update success\n");
#if (APP_FLASH_PROTECTION_ENABLE)
      flash_prot_op_cb(FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_END, 0, 0);
#endif
        //reboot
...
//		start_reboot();
      ...
      printf("[OTA][FLW] slave fw update failed\n");
        //erase the FW_UPDATE write area
        FW_UPDATE_FlashErase();
        //reboot
        irq_disable();
