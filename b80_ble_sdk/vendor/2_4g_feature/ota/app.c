...
#include "../../common/battery_check.h"
#include "mac.h"
#include "ota.h"
...
_attribute_data_retention_  volatile unsigned int ota_test_cnt = 0;
#endif
    ...
    ////////////////// OTA relative ////////////////////////
        ...
        flash_read_page(Flash_Addr,Flash_Buff_Len, (unsigned char *)Flash_Read_Buff);
          ...
          if(ota_test_cnt >= OTA_TEST_TIMES){
            printf("master OTA %d times,success %d times,fail %d times\n",ota_test_cnt,master_test_success_cnt,master_test_fail_cnt);
            printf("master OTA test end!\n");
...
//////////////////////////// User Configuration for 2.4G application ////////////////////////////
    ...
    #if(OTA_ROLE == MASTER)
    if(ota_test_cnt >= OTA_TEST_TIMES){
      printf("[OTA][FLW] master OTA %d times,success %d times,fail %d times\n",ota_test_cnt,master_test_success_cnt,master_test_fail_cnt);
      printf("[OTA][FLW] master OTA test end!\n");
      gpio_write(GPIO_PC6,1);
      while(1);
    }
    #endif
...
_attribute_ram_code_ void sdk_2p4g_main_loop(void)
{
    ...
    if((gpio_read(GPIO_PC6) == 1)){
        printf( "[OTA][FLW] slave OTA test end!\n");
        while(1);
  ...
  if (OTA_Trig)
  {
    ...
    flash_prot_op_cb(FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_BEGIN, 0, 0);
      ...
      MAC_Init(OTA_CHANNEL,
               ...
               OTA_RxTimeoutIrq);
#if (OTA_ROLE == MASTER)
      printf( "[OTA][FLW] master OTA START!\n");
      OTA_MasterInit(OTA_MASTER_BIN_ADDR, OTA_FW_VERSION);
#elif(OTA_ROLE == SLAVE)
      printf( "[OTA][FLW] slave OTA START!\n");
      flash_read_page(Flash_Addr,Flash_Buff_Len, (unsigned char *)Flash_Read_Buff);
        if (Flash_Read_Buff[0] == 0x4b)
        {
