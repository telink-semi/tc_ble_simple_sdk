...
//      printf( "[OTA][FLW] OTA_MASTER_STATE_IDLE %d\n",__LINE__);
      ...
      if(clock_time_exceed(state_tick, OTA_DURATION)){
        MasterCtrl.State = OTA_MASTER_STATE_ERROR;
        printf( "[OTA][FLW] ota time out%d\n",__LINE__);
        return;
      }
//      printf( "[OTA][FLW] OTA_MASTER_STATE_FW_VER_WAIT %d\n",__LINE__);
        if (Msg) {
//          printf( "[OTA][FLW] %d\n",__LINE__);
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
...
//      printf( "[OTA][FLW] OTA_MASTER_STATE_START_RSP_WAIT %d\n",__LINE__);
        if (Msg) {
//          printf( "[OTA][FLW] %d\n",__LINE__);
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
            //if receive a valid rf packet
...
//      printf( "[OTA][FLW] OTA_MASTER_STATE_DATA_ACK_WAIT %d\n",__LINE__);
        if (Msg) {
//          printf( "[OTA][FLW] %d\n",__LINE__);
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
//            //if receive a valid rf packet
...
//      printf( "[OTA][FLW] OTA_MASTER_STATE_END_RSP_WAIT %d\n",__LINE__);
        if (Msg) {
//          printf( "[OTA][FLW] %d\n",__LINE__);
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
            //if receive a valid rf packet
      ...
      printf( "[OTA][FLW] master ota success\n");
...
//            start_reboot();
        ...
        master_test_success_cnt++;
            ...
            cpu_sleep_wakeup(DEEPSLEEP_MODE_RET_SRAM_LOW16K, PM_WAKEUP_TIMER, ClockTime() + OTA_REBOOT_WAIT * 16);
#else
            start_reboot();
#endif
    }
    else if (OTA_MASTER_STATE_ERROR == MasterCtrl.State) {
      printf( "[OTA][FLW] master ota failed\n");
#if (APP_FLASH_PROTECTION_ENABLE)
      flash_prot_op_cb(FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_END, 0, 0);
#endif
#if(UI_LED_ENABLE)
...
//      printf( "[OTA][FLW] OTA_SLAVE_STATE_FW_VERSION_READY \n");
        if (Msg) {
//          printf( "[OTA][FLW] %d\n",__LINE__);
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
            //if receive a valid rf packet
...
//      printf( "[OTA][FLW] OTA_SLAVE_STATE_START_READY\n");
        if (Msg) {
//          printf( "[OTA][FLW] %d\n",__LINE__);
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
            //if receive a valid rf packet
...
//      printf( "[OTA][FLW] OTA_SLAVE_STATE_DATA_READY\n");
        if (Msg) {
//          printf( "[OTA][FLW] %d\n",__LINE__);
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
            //if receive a valid rf packet
...
//      printf( "[OTA][FLW] OTA_SLAVE_STATE_END_READY\n");
        if (Msg) {
//          tlkapi_send_string_data( "[OTA][FLW] ota packet", Msg, 8);
//          printf( "[OTA][FLW] %d\n",__LINE__);
            //if receive a valid rf packet
            if (Msg->Type == OTA_MSG_TYPE_DATA) {
				...
				RxFrame.Type = Msg->Data[1];
				memcpy(RxFrame.Payload, Msg->Data+2, Len-1);
//				tlkapi_send_string_data( "[OTA][FLW] MaxBlockNum", &SlaveCtrl.MaxBlockNum, 2);
...
//      printf( "[OTA][FLW] OTA_SLAVE_STATE_END\n");
#if 1
        // 1. todo FW crc check
...
//        printf("pkt_crc:%2x, fw_crc:%2x, target_fw_crc:%2x\r\n", SlaveCtrl.PktCRC, SlaveCtrl.FwCRC, SlaveCtrl.TargetFwCRC);
        ...
        {
            SlaveCtrl.State = OTA_SLAVE_STATE_ERROR;
            printf( "[OTA][FLW] Crc Check Error\n");
            return;
        }
#endif
...
//        }
        printf( "[OTA][FLW] slave ota success\n");
#if (APP_FLASH_PROTECTION_ENABLE)
        flash_prot_op_cb(FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_END, 0, 0);
#endif
        gpio_set_func(GPIO_LED_WHITE, AS_GPIO);
...
//        start_reboot();
      ...
      printf( "[OTA][FLW] OTA_SLAVE_STATE_ERROR\n");
        //erase the OTA write area
        OTA_FlashErase();
        irq_disable();
        printf( "[OTA][FLW] slave ota failed\n");
#if (APP_FLASH_PROTECTION_ENABLE)
        flash_prot_op_cb(FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_END, 0, 0);
#endif
