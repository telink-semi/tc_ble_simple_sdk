...
void flash_protection_init(void)
{
	...
	switch(blc_flash_mid)
	{
		...
		#if (FLASH_GD25WD10EGIG_SUPPORT_EN)
			case MID1164C8:
				flash_lock_mid = (flash_lock_t)flash_lock_mid1164c8;
				flash_unlock_mid = flash_unlock_mid1164c8;
				flash_get_lock_status_mid = (flash_get_lock_status_t)flash_get_lock_block_mid1164c8;
				flash_unlock_status = FLASH_LOCK_NONE_MID1164C8;
				break;
		#endif

		#if(FLASH_ZG25WD40B_SUPPORT_EN)
			case MID13325E:
		...
		default:
			blc_flashProt.init_err = 1; //Do not support this flash.
			break;
	}
}
	...
	switch(blc_flash_mid)
	{
			...
			case MID1164C8:
				if(app_lock_block == FLASH_LOCK_FW_LOW_128K){
					 ...
					 * 			  now firmware size under protection is 128K - 64K = 64K
					 * if this demo can not meet your requirement, you should change !!! */
					flash_lock_block_size = FLASH_LOCK_LOW_64K_MID1164C8;
					printf("[FLASH][PROT] flash lock low 64K block!\n");
				}
				else{
					blc_flashProt.init_err = 1; //can not use LOCK LOW 1M for 512K capacity flash
		...
		#if (FLASH_ZG25WD40B_SUPPORT_EN) //512K capacity
			case MID13325E:
				if(app_lock_block == FLASH_LOCK_FW_LOW_256K){
