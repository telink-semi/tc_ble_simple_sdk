...
_attribute_data_retention_	rf_private_pa_callback_t  rf_private_pa_cb = 0;
...
void rf_pa_init(void)
{
    ...
    rf_private_pa_cb = app_rf_pa_handler;
#endif
}
...
_attribute_ram_code_sec_ void rf_pa_handler(short int flag)
{
	...
	case FLD_RF_IRQ_TX:
				...
				{
				    rf_private_pa_cb(PRIVATE_PA_TYPE_OFF);
				}
				else if (gen_fsk_current_mode == GEN_FSK_MD_STX2RX)
				{
				    rf_private_pa_cb(PRIVATE_PA_TYPE_RX_ON);
				}
                break;
	case FLD_RF_IRQ_RX:
				...
				{
				    rf_private_pa_cb(PRIVATE_PA_TYPE_TX_ON);
				}
                break;
